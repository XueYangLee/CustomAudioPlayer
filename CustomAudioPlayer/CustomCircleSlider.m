...
@implementation CustomCircleSlider
...
- (void)setup {
    ...
    self.sliderWidth = 5.0f;
    self.thumbRadius = 12.0f;
    self.thumbExpandRadius = 25.0f;
    self.bufferTrackTintColor = [UIColor lightGrayColor];
    ...
    [self addTarget:self action:@selector(circleSliderTouchDown:) forControlEvents:UIControlEventTouchDown];
    [self addTarget:self action:@selector(circleSliderValueChanging:) forControlEvents:UIControlEventValueChanged];
    [self addTarget:self action:@selector(circleSliderValueDidChanged:) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetLineWidth(ctx, self.sliderWidth);
    ...
    CGFloat loadCurre = loadStart + 2 * M_PI * self.bufferValue;
    ...
    CGContextSetLineWidth(ctx, self.sliderWidth);
    CGContextSetStrokeColorWithColor(ctx, self.bufferTrackTintColor.CGColor);
    ...
    CGContextSaveGState(ctx);
    CGContextSetShouldAntialias(ctx, YES);
    CGContextSetFillColorWithColor(ctx, self.maximumTrackTintColor.CGColor);
    CGContextAddArc(ctx, self.circleStartPoint.x, self.circleStartPoint.y, self.sliderWidth / 2.0, 0, M_PI * 2, 0);
...
- (BOOL)beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    double touchDist = [CustomCircleSlider distanceBetweenPointA:starTouchPoint pointB:self.lastPoint];
    if (touchDist > 44) {
        self.interaction = NO;
    ...
    double dist = [CustomCircleSlider distanceBetweenPointA:starTouchPoint pointB:self.drawCenter];
...
- (void)moveHandlerWithPoint:(CGPoint)point {
    ...
    CGFloat angle = [CustomCircleSlider calculateAngleWithRadius:self.radius
...
+ (CGFloat)calculateAngleWithRadius:(CGFloat)radius
    ...
    //a^2 = b^2 + c^2 - 2bccosA;
    CGFloat cosA = (2 * radius * radius - powf([CustomCircleSlider distanceBetweenPointA:startCenter pointB:endCenter], 2)) / (2 * radius * radius);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    CustomCircleSlider *circleSlider = (CustomCircleSlider *)object;
...
- (void)circleSliderTouchDown:(CustomCircleSlider *)slider{
    if (!slider.interaction) {
        return;
    }
    if (self.sliderTouchDown) {
        self.sliderTouchDown(slider);
    }
}

- (void)circleSliderValueChanging:(CustomCircleSlider *)slider{
    if (!slider.interaction) {
        return;
    }
    if (self.sliderValueChanging) {
        self.sliderValueChanging(slider, slider.value);
    }
}

- (void)circleSliderValueDidChanged:(CustomCircleSlider *)slider{
    if (!slider.interaction) {
        return;
    }
    if (self.sliderValueChanged) {
        self.sliderValueChanged(slider, slider.value);
    }
}
